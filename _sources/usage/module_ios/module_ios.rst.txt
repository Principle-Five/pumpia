...
Outputs are reset on analysis unless the ``reset_on_analysis`` parameter is set to ``False``.
This should be noted for any modules where the output value is generated during the ``draw_rois`` step.
